# -*- coding: utf-8 -*-
"""
Created on Tue Sep 22 18:43:27 2020
...
from customoptimizers import ConstIntervalOptimizer, VariableIntervalOptimizer, ConstIntervalMemoryOptimizer
...
for i in range(3):
    nets[i]['name'] = f'CONSTANT JUMP INTERVAL EXPONENT\ninterval:{intervals[i]} expon scale:{1/expon_scale[i]}'
...
mean = [0 for i in range(3)]
std = [sqrt(alpha_0) for i in range(3)]
intervals = [1, 50, 100]
# create all the constant interval networks
for i in range(3,6):
    nets[i]['name'] = f'Constant jump interval Normal\ninterval:{intervals[i-3]} mean:{mean[i-3]} std:{std[i-3]}'
    nets[i]['plotter'] = Plotter(interval=100, title=nets[i]['name'])
    nets[i]['generator'] = NormalGenerator(mean=mean[i-3], std=std[i-3])
    nets[i]['optimizer'] = ConstIntervalOptimizer(nets[i]['net'].parameters(),
                                                  nets[i]['generator'], lr=alpha_0, interval=intervals[i-3])

#-----------------------------------variable time interval networks-----------

hop_size = [10**-i for i in range(1, 4)]
jump_prob = [0.1*i for i in range(1, 4)]
# create all the constant interval networks
for i in range(6,9):
    nets[i]['name'] = f'VARIABLE JUMP INTERVAL\nsize:{hop_size[i-6]} jump prob:{jump_prob[i-6]}'
    nets[i]['plotter'] = Plotter(interval=100, title=nets[i]['name'])
    nets[i]['optimizer'] = VariableIntervalOptimizer(nets[i]['net'].parameters(), lr=alpha_0,
                                                     hop_size=hop_size[i-6], jump_prob=jump_prob[i-6])
